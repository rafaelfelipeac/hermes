...
- Enabling slot-aware drag-and-drop incrementally can be done without a full section-key rewrite by deriving slot from drop position within each day section (top/middle/bottom thirds) and reusing existing day-level hit testing; this keeps the interaction shippable while larger DnD refactors remain optional.
